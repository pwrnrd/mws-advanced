const { transformObjectKeys, forceArray } = require('../util/transformers');
...
const parseMarketplaceData = (marketplaceData) => {
    ...
    marketDetails = forceArray(marketsTemp).reduce((acc, market) => {
    ...
    marketDetails = forceArray(marketParticipationsTemp).reduce((acc, participation) => {
