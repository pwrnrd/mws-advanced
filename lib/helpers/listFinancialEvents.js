...
const outParserListFinancialEvents = (out) => out.FinancialEvents;

const outputParserWithNextToken = (out) => ({
    // TODO: Need to parse the output for the different transaction types
    nextToken: out.NextToken,
    result: out.FinancialEvents,
});

const listFinancialEvents = (api) => api.parseEndpoint(outParserListFinancialEvents, inputParser)('ListFinancialEvents');

const listFinancialEventsByNextToken = (api) => api.parseEndpoint(outputParserWithNextToken)('ListFinancialEventsByNextToken');

const listFinancialEventsAll = (api) => async (options = {}) => {
    let results = [];
    const financialEvents = await api.parseEndpoint(outputParserWithNextToken, inputParser)('ListFinancialEvents')(options);
